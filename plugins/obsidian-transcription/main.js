  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setClass("swiftink-settings").setName("Swiftink Account").addButton((bt) => {
      ...
      bt.onClick(async () => {
        ...
        await this.plugin.saveSettings();
        containerEl.findAll(".swiftink-unauthed-only").forEach((element) => {
          element.style.display = "block";
        });
    ...
    new import_obsidian.Setting(containerEl).setName("Enable timestamps").setDesc("Add timestamps to the beginning of each line").setClass("swiftink-settings").addToggle((toggle) => toggle.setValue(this.plugin.settings.timestamps).onChange(async (value) => {
      this.plugin.settings.timestamps = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Timestamp format").setDesc("Your choice of hours, minutes, and/or seconds in the timestamp").setClass("swiftink-settings").addDropdown((dropdown) => dropdown.addOption("HH:mm:ss", "HH:mm:ss").addOption("mm:ss", "mm:ss").addOption("ss", "ss").setValue(this.plugin.settings.timestampFormat).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Whisper ASR URL").setDesc("The URL of the Whisper ASR server: https://github.com/ahmetoner/whisper-asr-webservice").setClass("whisper-asr-settings").addText((text) => text.setPlaceholder(DEFAULT_SETTINGS.whisperASRUrl).setValue(this.plugin.settings.whisperASRUrl).onChange(async (value) => {
...
function _regeneratorRuntime2() {
  ...
  function AsyncIterator(generator, PromiseImpl) {
    function invoke(method, arg, resolve, reject) {
      ...
      if (record.type !== "throw") {
        var result = record.arg, value = result.value;
        return value && _typeof9(value) == "object" && hasOwn.call(value, "__await") ? PromiseImpl.resolve(value.__await).then(function(value2) {
          invoke("next", value2, resolve, reject);
        }, function(err) {
          invoke("throw", err, resolve, reject);
        }) : PromiseImpl.resolve(value).then(function(unwrapped) {
...
`;
  ...
  async getTranscriptionWhisperASR(file) {
    ...
    return (0, import_obsidian3.requestUrl)(options).then(async (response) => {
      if (this.settings.debug)
      ...
      class SwiftinkTranscriptFunctionsModal extends import_obsidian4.FuzzySuggestModal {
        ...
        onChooseItem(function_name) {
          if (function_name == "View on Swiftink.io") {
            window.open("https://swiftink.io/dashboard/", "_blank");
          }
        }
      }
